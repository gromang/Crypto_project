- [Crypto_parser](#crypto_parser)
- [Определение требуемых данных для построения графика цены](#Определение-требуемых-данных-для-построения-графика-цены)
- [Какие данные может отдать биржа?](#Какие-данные-может-отдать-биржа?)
- [Разбор возвращаемых данных по каждой бирже:](#Разбор-возвращаемых-данных-по-каждой-бирже)
  * [Binance :](#Binance-возвращает-список-списков-следующего-вида)
  * [HitBTC:](#Hitbtc-возвращает-JSON-вида)
  * [BitFinex:](#BITFINEX-возвращает-список-списков-следующего-вида)
  * [Huobi:](#HUOBI-возвращает-JSON-следующего-вида)
  * [Kraken:](#KRAKEN-возвращает-массив-следующего-вида)
- [Выбор данных для хранения и обработки](#Выбор-данных-для-хранения-и-обработки)
- [База данных](#База-данных)


## Crypto_parser

Crypto_parser предназначен для сбора котировок с нескольких крупных криптобирж.
На данный момент в проекте реализован парсинг 5 бирж : Binance, HitBTC, Bitfinex, Huobi и Kraken. Парсятся наиболее ликвидные инструменты на текущий момент: BTCUSD, LTCUSD, ETHUSD, XRPUSD. Объектом парсинга являются данные минутных свечей. Эти данные затем агрегируются (суммируются по весовым коэффициентам) для каждой криптовалюты, и итоговые минутные свечи складываются в базу данных.

> Сам модуль является лишь частью сервиса Diagram, задача которого - построение графиков из полученных данных
...
## Выбор данных для хранения и обработки

1. Для обработки нам понадобится с каждого api забирать данные предыдущего заданного интервала (в проекте используется 1 минута). Необходимые данные:

* Open
* Close
* High
* Low
* Volume ( базового актива )
* Timestamp

2. Далее каждый элемент этого списка приводится к единому типу данных
...
3. Затем происходит формирование параметров итоговой свечи/бара

  Для этого проводим суммирование с поправкой на объём (тем самым вычисляя вес каждого параметра при формировании итогового параметра свечи ).
  Для сокращения примем следующие обозначения:
  * V, OPEN  - объём и цена открытия итоговой свечи
  * vb, vh и vk - объёмы свечей, полученных соотвественно от Binance, HitBtc и Kraken
  * ob, oh и ok - цены открытия свечей, полученных соотвественно от Binance, HitBtc и Kraken

  Тогда:
  ```
   Объем итоговой свечи: 
        V = vb + vh + ... + vk
   Цена открытия итоговой свечи:
        OPEN = ob*( vb/V ) + oh*(vh/V) + ...+ ok*( vk/V )
  ```
  По этому же принципу высчитываются цены CLOSE, LOW и HIGH.
  Timestamp итоговой свечи должен совпадать с timestamp получаемых по api свечей.
  Если время не совпадает, данная котировка не участвует в расчете.

4. Параметры итоговой свечи заносятся в базу.
...
Парсер запускается по расписаню каждую минуту, опрашивает api, получает данные и заносит их в базу.
